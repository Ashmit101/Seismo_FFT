...
def separate_directions(data, sampling_rate, x_orientation, z_orientation):
    ...
    for direction in ["east", "west", "north", "south"]:
        _, directions[direction]["fft"] = apply_fft(
            directions[direction]["signal"], sampling_rate
        )
...
def plot_directional_analysis(directions, time):
    ...
    plt.subplot(3, 1, 3)
    plt.plot(
        directions["north"]["freq"], directions["north"]["fft"], "g", label="North"
    )
    plt.plot(
        directions["south"]["freq"], directions["south"]["fft"], "y", label="South"
    )
...
def main():
    ...
    directions, time = separate_directions(
        data, sampling_rate, x_orientation, z_orientation
    )
